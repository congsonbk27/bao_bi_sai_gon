...
export const LICENSE_ANSWER_CODE =  4
...
export const getAnswerCode = async () => {
  const where = { setting_id: LICENSE_ANSWER_CODE }
  const settings = await db.settings
    .find(where)
    .limit(1)
  return settings ? settings[0] : {}
...
export const setAnswerCode = async (code) => {
  const where = { setting_id: LICENSE_ANSWER_CODE }
  const res = await db.settings
    .update(where, { $set: { value: code } }, {})
  return res
}
