...
import md5 from 'md5'
import { Button, Select, Card, Icon, Input, Tooltip } from 'antd'
...
export default class Page extends React.Component {
  ...
  saveCOMport = async () => {
    if (!this.state.port) {
      alert('Vui lòng chọn cổng trước khi bấm lưu')  
      return
    }
    try {
      await setPort(this.state.port)
      alert('Cài đặt cổng máy in thành công')  
    } catch (error) {
      alert('Cài đặt cổng máy in thất bại' + error.message)  
    }
  }
  
  saveHomePage = async () => {
    if (!this.state.defaultPage) {
      alert("Hãy chọn trang chủ mặc định trước khi bấm lưu")
      return
    }
    try {
      await setDefaultPage(this.state.defaultPage)
      alert("Thay trang chủ mặc định thành công")
    } catch (error) {
      alert("Thay trang chủ mặc định thất bại")
    }
  }
  ...
  savePassword = async () => {
    ...
    if (!this.state.rePassword || this.state.rePassword !== this.state.password) {
      alert('Vui lòng nhập lại mật khẩu mới! 2 mật khẩu không khớp!')
      return
    }
    if (!this.state.port) {
      alert('Vui lòng chọn cổng trước khi bấm lưu')  
